...
async function geocodeVenueAddress(venue) {
  const parts = [venue.street, venue.city, venue.state, "USA"].filter(Boolean);
  ...
  try {
    ...
    if (!isValidCoordinate(lat, lng)) throw new Error("Address lookup returned invalid coordinates");
...
async function chooseVenue(venue) {
  ...
  els.barCity.value = selectedVenue.city || "";
...
async function submitColdie(event) {
  ...
  if (hasSelectedVenue && !isValidCoordinate(venueLat, venueLng)) {
    showSubmitMessage("That selected venue is missing a valid map location. Try selecting it again, or edit the venue name to submit it for admin review.");
    els.venueSearch.focus();
    return;
  }
  ...
  const payload = {
    ...
    state: hasSelectedVenue ? (els.barState.value.trim() || selectedVenue.state || "") : "",
