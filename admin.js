...
async function loadQueue() {
  ...
  rows.forEach(r => {
    ...
    document.getElementById(`reject-${r.id}`).addEventListener("click", () => rejectSubmission(r));
  });
}

function cardHtml(r, photoUrl) {
  return `<article class="review-card" id="card-${r.id}">
    <div class="photo-wrap">${photoUrl ? `<img src="${escapeHtml(photoUrl)}" alt="Submitted thermometer evidence" />` : '<div class="photo-placeholder">Thermometer photo unavailable</div>'}</div>
    <div class="review-body">
      <div class="review-top">
        <div><h3>${escapeHtml(r.bar_name)}</h3><div class="beer">${escapeHtml(r.beer_name)}</div></div>
        <div class="temp">${Math.round(Number(r.temperature_f))}°F</div>
      </div>
      <div class="meta">${escapeHtml(r.address)} · ${escapeHtml(r.city)}, ${escapeHtml(r.state || "CA")}</div>
      <div class="meta">Measured ${escapeHtml(formatWhen(r.measured_at))}</div>
      ...
      <div class="location-row">
        <button class="secondary" id="locate-${r.id}">📍 Locate bar</button>
        <div class="location-status" id="location-${r.id}">Required before approving a new location.</div>
      </div>
      <div class="actions">
        <button class="danger" id="reject-${r.id}">Reject</button>
        <button class="approve" id="approve-${r.id}" disabled>Approve</button>
      </div>
    </div>
  </article>`;
}
...
async function locateSubmission(r) {
  ...
  try {
    const q = encodeURIComponent(`${r.address}, ${r.city}, ${r.state || "CA"}, USA`);
    const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=us&q=${q}`, {headers:{"Accept":"application/json"}});
    if (!response.ok) throw new Error(`Geocoder returned ${response.status}`);
    const found = await response.json();
    if (!found.length) throw new Error("Address not found");
    const lat = Number(found[0].lat), lng = Number(found[0].lon);
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new Error("Invalid coordinates");
    geocodes.set(r.id, {lat, lng});
    status.textContent = `Found: ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
...
async function rejectSubmission(r) {
  if (!confirm(`Reject ${r.bar_name} — ${r.beer_name} at ${r.temperature_f}°F?`)) return;
  const card = document.getElementById(`card-${r.id}`);
  card.classList.add("busy");
  const { error } = await client.rpc("reject_submission", {p_submission_id:r.id, p_review_notes:null});
  if (error) {
    card.classList.remove("busy");
